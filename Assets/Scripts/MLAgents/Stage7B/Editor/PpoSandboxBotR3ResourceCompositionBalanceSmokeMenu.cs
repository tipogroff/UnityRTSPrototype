...
{
    [InitializeOnLoad]
    public static class PpoSandboxBotR3ResourceCompositionBalanceSmokeMenu
    {
        private const string MenuPath = "RTS/Week7/PPO Sandbox/Run Bot R3 Resource+Composition Balance Smoke";
        private const string ScenePath = "Assets/Scenes/PPO_TrainingSandbox_Player1AI_vs_ScriptedBot.unity";
        private const string ArtifactFileName = "ppo_sandbox_bot_r3_resource_composition_balance_smoke_runtime.json";
        private const string PendingKey = "RTS.Stage7B.PPO.BotR3ResourceCompositionBalance.Pending";
        private const string StartedAtTicksKey = "RTS.Stage7B.PPO.BotR3ResourceCompositionBalance.StartedAtTicks";
        private const double TimeoutSeconds = 90d;

        static PpoSandboxBotR3ResourceCompositionBalanceSmokeMenu()
        {
        ...
        public static void Run()
        {
            ...
            {
                Debug.LogWarning("[PPO-SANDBOX-BOT-R3] Smoke run must be started from Edit Mode.");
                return;
            }

            if (SessionState.GetBool(PendingKey, false))
            {
                Debug.LogWarning("[PPO-SANDBOX-BOT-R3] Smoke run is already pending.");
                return;
            }

            if (EditorSceneManager.GetActiveScene().isDirty)
            {
                Debug.LogError("[PPO-SANDBOX-BOT-R3] Save or revert current scene before running smoke.");
                return;
            }

            if (EditorSceneManager.OpenScene(ScenePath) == default)
            {
                Debug.LogError("[PPO-SANDBOX-BOT-R3] Failed to open sandbox scene.");
                return;
            }
            ...
            Debug.Log("[PPO-SANDBOX-BOT-R3] Entering Play Mode for resource/composition balance smoke.");
        ...
        private static void OnPlayModeStateChanged(PlayModeStateChange state)
        {
            ...
            if (!File.Exists(artifactPath))
            {
                ...
                if (!File.Exists(artifactPath))
                {
                    Debug.LogError("[PPO-SANDBOX-BOT-R3] Smoke run ended without runtime artifact.");
                    return;
                }
            }

            Debug.Log("[PPO-SANDBOX-BOT-R3] Smoke run finished. Artifact: " + ArtifactFileName);
        }
        ...
        private static void WriteRuntimeArtifact()
        {
            ...
            Debug.Log("[PPO-SANDBOX-BOT-R3] Wrote runtime artifact.");
        }
