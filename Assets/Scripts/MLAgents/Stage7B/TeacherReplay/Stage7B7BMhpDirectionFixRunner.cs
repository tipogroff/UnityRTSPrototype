...
namespace RTS.MLAgents.Stage7B.TeacherReplay
{
    ...
    public sealed class Stage7B7BMhpDirectionFixRunner : MonoBehaviour
    {
        ...
        private const string ReportJsonPath = "python/stage7b_teacher_replay/stage7b_7b_mhp_direction_fix_report.json";
        private const string ReportMdPath = "python/stage7b_teacher_replay/stage7b_7b_mhp_direction_fix_report.md";
        private const string MoveMismatchesPath = "python/stage7b_teacher_replay/stage7b_7b_move_remaining_mismatches.jsonl";
        private const string HarvestMismatchesPath = "python/stage7b_teacher_replay/stage7b_7b_harvest_remaining_mismatches.jsonl";
        private const string ProduceMismatchesPath = "python/stage7b_teacher_replay/stage7b_7b_produce_remaining_mismatches.jsonl";
        private const string RemainingMismatchesPath = "python/stage7b_teacher_replay/stage7b_7b_remaining_mismatches.jsonl";
        private const string RuntimeApplyTracePath = "python/stage7b_teacher_replay/stage7b_7b_runtime_apply_trace.jsonl";
        private const string MappingTracePath = "python/stage7b_teacher_replay/stage7b_7b_mapping_trace.jsonl";

        private const int CandidateMatchCountBefore7A = 2396;
        private const float CandidateMatchRateBefore7A = 0.811653f;
        private const float RuntimeApplyAcceptRateBefore7A = 1.0f;
        private const float MoveMatchRateBefore7A = 0.890479f;
        private const int MoveDirectionMismatchCountBefore7A = 222;
        private const float HarvestMatchRateBefore7A = 0.668831f;
        private const int HarvestDirectionMismatchCountBefore7A = 51;
        private const float ProduceMatchRateBefore7A = 0.555730f;
        private const int ProduceDirectionMismatchCountBefore7A = 26;
        ...
        [ContextMenu("Run Stage7B-7B M/H/P Direction Fix")]
        public void RunStage7B7BMhpDirectionFix()
        {
            var report = Stage7B7BMhpDirectionFixReport.CreateDefault();
            report.generated_at_utc = DateTime.UtcNow.ToString("yyyy-MM-ddTHH:mm:ssZ");
            report.source_path = SourceDir;
            report.summary = "Stage7B-7B Move/Harvest/Produce direction mapping fix validation.";
            report.cardinal_direction_mapping_mode = Stage7BTeacherReplayActionResolver.CardinalDirectionMappingModeInvertYForLegacy032Teacher;
            report.notes.Add("Mapping-only validation: no ML-Agents training, PPO, imitation learning, or large .demo recording was started.");
            ...
            report.notes.Add("Teacher Replay resolver maps Move/Harvest/Return/Produce cardinal direction branches with legacy032 vertical-axis conversion.");

            var moveLines = new List<string>(256);
            var harvestLines = new List<string>(64);
            var produceLines = new List<string>(320);
            var remainingMismatchLines = new List<string>(640);
            var applyTraceLines = new List<string>(4096);
            var mappingTraceLines = new List<string>(4096);
            ...
            if (!_loader.TryLoadReplayManifest(SourceDir, out Stage7BTeacherReplayManifest manifest, out string manifestDiag))
            {
                ...
                WriteArtifacts(report, moveLines, harvestLines, produceLines, remainingMismatchLines, applyTraceLines, mappingTraceLines);
                return;
            }
            ...
            for (int stepIndex = 0; stepIndex < steps.Count; stepIndex++)
            {
                ...
                for (int commandIndex = 0; commandIndex < commands.Length; commandIndex++)
                {
                    ...
                    var applyTrace = new Stage7B7BRuntimeApplyTraceEntry
                    {
                    ...
                    Stage7B7BMappingTraceEntry mappingTrace = null;
                    ...
                    CopyMappingDiagnostics(command, resolver, applyTrace);
                    if (IsDirectionMappedAction(command.action_type))
                    {
                        RecordMappingApplied(report, resolver);
                        mappingTrace = BuildMappingTrace(step, commandIndex, command, resolver, teacherAction);
                    }
                    ...
                    if (!matcher.TryMatch(teacherAction, candidates, out int candidateIndex, out Stage7BTeacherReplayDropReason matchDrop))
                    {
                        ...
                        {
                            mappingTrace.candidate_match = false;
                            mappingTrace.drop_reason = ToSnakeCase(matchDrop);
                            mappingTrace.nearest_candidate_reason = nearestReason;
                            mappingTraceLines.Add(JsonUtility.ToJson(mappingTrace));
                        }

                        if (IsDirectionMappedAction(command.action_type))
                        {
                            Stage7B7BRemainingMismatchEntry mismatch = BuildMismatchEntry(
                                step,
                                commandIndex,
                                command,
                                teacherAction,
                            ...
                            RecordMismatch(report, mismatch, moveLines, harvestLines, produceLines, remainingMismatchLines);
                        }
                    ...
                    if (mappingTrace != null)
                    {
                        mappingTrace.candidate_match = true;
                        mappingTrace.candidate_action_index = candidateIndex;
                    }
                    ...
                    if (applied)
                    {
                        ...
                        if (mappingTrace != null)
                        {
                            mappingTrace.runtime_apply_attempted = true;
                            mappingTrace.runtime_apply_accepted = true;
                        }
                    }
                    else
                    {
                        ...
                        {
                            mappingTrace.runtime_apply_attempted = true;
                            mappingTrace.runtime_apply_accepted = false;
                            mappingTrace.reject_reason = rejectReason;
                        }
            ...
            report.demo_recording_ready_after_7b = report.contract_error_count == 0
                && report.state_sync_failed_count == 0
                && report.runtime_apply_attempted_count > 0
                && report.runtime_apply_rejected_count == 0
                && report.runtime_apply_accept_rate >= 0.999f
                && report.candidate_match_rate > CandidateMatchRateBefore7A
                && report.move_match_rate > MoveMatchRateBefore7A
                && report.harvest_match_rate > HarvestMatchRateBefore7A
                && report.produce_direction_mismatch_count < ProduceDirectionMismatchCountBefore7A
                && report.return_match_rate >= 1.0f
                && report.return_direction_mismatch_count == 0;

            bool go = report.demo_recording_ready_after_7b;

            report.direction_pattern_conclusion = BuildDirectionConclusion(report);
            report.decision = go
                ? "GO_TO_STAGE7B_8_SMALL_IMITATION_SMOKE"
                : "HOLD_FOR_ADDITIONAL_FIX";
            report.status = go ? "GO" : "NO_GO";

            if (!go)
            {
                report.notes.Add("Stage7B-7B GO criteria were not all met; hold before Stage7B-8.");
            }
            else
            {
                report.notes.Add("Stage7B-7B GO criteria met for Stage7B-8 small imitation smoke.");
            }
        ...
        private void WriteArtifacts(
            Stage7B7BMhpDirectionFixReport report,
            ...
            List<string> mappingTraceLines)
        {
            ...
            _loader.TrySaveText(RemainingMismatchesPath, string.Join("\n", remainingMismatchLines), out _);
            _loader.TrySaveText(RuntimeApplyTracePath, string.Join("\n", applyTraceLines), out _);
            _loader.TrySaveText(MappingTracePath, string.Join("\n", mappingTraceLines), out _);
            ...
            Debug.Log("[Stage7B][7B] M/H/P direction fix report written: " + jsonPath);
        }

        private static void FinalizeReport(Stage7B7BMhpDirectionFixReport report, List<int> candidateCounts)
        {
            ...
            report.return_commands_dropped = Math.Max(0, report.return_commands_total - report.return_commands_matched);
            ...
            report.return_match_rate = SafeRate(report.return_commands_matched, report.return_commands_total);
            ...
            report.return_direction_mismatch_rate = SafeRate(report.return_direction_mismatch_count, report.return_commands_total);

            report.candidate_match_count_before_7a = CandidateMatchCountBefore7A;
            report.candidate_match_count_after_7b = report.candidate_match_count;
            report.candidate_match_rate_before_7a = CandidateMatchRateBefore7A;
            report.candidate_match_rate_after_7b = report.candidate_match_rate;
            report.runtime_apply_accept_rate_before_7a = RuntimeApplyAcceptRateBefore7A;
            report.runtime_apply_accept_rate_after_7b = report.runtime_apply_accept_rate;
            report.runtime_apply_rejected_count_after_7b = report.runtime_apply_rejected_count;
            report.state_sync_failed_count_after_7b = report.state_sync_failed_count;

            report.move_match_rate_before_7a = MoveMatchRateBefore7A;
            report.move_match_rate_after_7b = report.move_match_rate;
            report.move_direction_mismatch_count_before_7a = MoveDirectionMismatchCountBefore7A;
            report.move_direction_mismatch_count_after_7b = report.move_direction_mismatch_count;

            report.harvest_match_rate_before_7a = HarvestMatchRateBefore7A;
            report.harvest_match_rate_after_7b = report.harvest_match_rate;
            report.harvest_direction_mismatch_count_before_7a = HarvestDirectionMismatchCountBefore7A;
            report.harvest_direction_mismatch_count_after_7b = report.harvest_direction_mismatch_count;

            report.produce_match_rate_before_7a = ProduceMatchRateBefore7A;
            report.produce_match_rate_after_7b = report.produce_match_rate;
            report.produce_direction_mismatch_count_before_7a = ProduceDirectionMismatchCountBefore7A;
            report.produce_direction_mismatch_count_after_7b = report.produce_direction_mismatch_count;
            report.produce_type_mismatch_count_after_7b = report.produce_type_mismatch_count;
            report.action_type_missing_from_candidates_after_7b = report.action_type_missing_from_candidates_count;

            report.return_match_rate_after_7b = report.return_match_rate;
            report.return_direction_mismatch_count_after_7b = report.return_direction_mismatch_count;
        ...
        private static void CopyMappingDiagnostics(
            Stage7BTeacherReplayTeacherCommand command,
            Stage7BTeacherReplayActionResolver resolver,
            Stage7B7BRuntimeApplyTraceEntry applyTrace)
        {
            if (command == null || resolver == null || applyTrace == null) return;
            applyTrace.raw_teacher_dir = resolver.LastRawTeacherDir;
            applyTrace.mapped_unity_dir = resolver.LastMappedUnityDir;
            applyTrace.mapping_mode = resolver.LastCardinalDirectionMappingMode;
            applyTrace.mapping_applied = resolver.LastCardinalDirectionMappingApplied;
        }

        private static Stage7B7BMappingTraceEntry BuildMappingTrace(
        ...
        {
            return new Stage7B7BMappingTraceEntry
            {
                episode_id = step.episodeId,
                step_id = step.stepId,
                command_index = commandIndex,
                actor_flat = command.actor_flat,
                actor_x = teacherAction.ActorPosition.X,
                actor_y = teacherAction.ActorPosition.Y,
                action_type = command.action_type,
                action_type_name = ActionTypeToString(command.action_type),
                raw_teacher_dir = resolver.LastRawTeacherDir,
                mapped_unity_dir = resolver.LastMappedUnityDir,
                mapped_unity_dir_name = teacherAction.Direction.ToString(),
                mapping_mode = resolver.LastCardinalDirectionMappingMode,
                mapping_applied = resolver.LastCardinalDirectionMappingApplied,
                resolved = true,
                candidate_action_index = -1,
            };
        }

        private static void RecordMappingApplied(
            Stage7B7BMhpDirectionFixReport report,
            Stage7BTeacherReplayActionResolver resolver)
        {
            if (report == null || resolver == null || !resolver.LastCardinalDirectionMappingApplied) return;
            string actionType = resolver.LastMappingAppliedActionType.ToString().ToLowerInvariant();
            AddHistogram(report.mapping_applied_action_types, actionType);

            switch (resolver.LastMappingAppliedActionType)
            {
                case UnitActionType.Move:
                    report.move_mapping_applied_count++;
                    break;
                case UnitActionType.Harvest:
                    report.harvest_mapping_applied_count++;
                    break;
                case UnitActionType.Return:
                    report.return_mapping_applied_count++;
                    break;
                case UnitActionType.Produce:
                    report.produce_mapping_applied_count++;
                    break;
            }
        }

        private static Stage7B7BRemainingMismatchEntry BuildMismatchEntry(
            Stage7BTeacherTrajectoryStep step,
            int commandIndex,
            Stage7BTeacherReplayTeacherCommand command,
            AgentAction teacherAction,
            ...
            ResourceManager resources)
        {
            ...
            return new Stage7B7BRemainingMismatchEntry
            {
                episode_id = step.episodeId,
                step_id = step.stepId,
                command_index = commandIndex,
        ...
        private static void RecordMismatch(
            Stage7B7BMhpDirectionFixReport report,
            Stage7B7BRemainingMismatchEntry mismatch,
            List<string> moveLines,
            List<string> harvestLines,
            List<string> produceLines,
            List<string> remainingMismatchLines)
        {
            ...
            bool actionTypeMissing = mismatch.nearest_candidate_reason != null
                && mismatch.nearest_candidate_reason.StartsWith("action_type_missing_from_candidates", StringComparison.Ordinal);

            remainingMismatchLines.Add(JsonUtility.ToJson(mismatch));
            AddHistogram(report.remaining_mismatch_reason_histogram, NormalizeNearestReason(mismatch.nearest_candidate_reason));
            ...
            else if (mismatch.action_type == 4)
            {
                ...
                if (actionTypeMissing) report.action_type_missing_from_candidates_count++;
            ...
            else if (mismatch.action_type == 3)
            {
                if (directionMismatch) report.return_direction_mismatch_count++;
            }
        }

        private static string NormalizeNearestReason(string nearestReason)
        {
            if (string.IsNullOrWhiteSpace(nearestReason)) return "unknown";
            int paren = nearestReason.IndexOf(" (", StringComparison.Ordinal);
            return paren > 0 ? nearestReason.Substring(0, paren) : nearestReason;
        }

        private static void AddFirst10(List<string> target, Stage7B7BRemainingMismatchEntry mismatch)
        {
        ...
        private static void IncrementActionTotal(Stage7B7BMhpDirectionFixReport report, int actionType)
        {
            if (actionType == 1) report.move_commands_total++;
            else if (actionType == 2) report.harvest_commands_total++;
            else if (actionType == 3) report.return_commands_total++;
            else if (actionType == 4) report.produce_commands_total++;
        }

        private static void IncrementActionMatched(Stage7B7BMhpDirectionFixReport report, int actionType)
        {
            if (actionType == 1) report.move_commands_matched++;
            else if (actionType == 2) report.harvest_commands_matched++;
            else if (actionType == 3) report.return_commands_matched++;
            else if (actionType == 4) report.produce_commands_matched++;
        }

        private static bool IsDirectionMappedAction(int actionType)
        {
            return actionType == 1 || actionType == 2 || actionType == 3 || actionType == 4;
        }
        ...
        private static string BuildDirectionConclusion(Stage7B7BMhpDirectionFixReport report)
        {
        ...
        private static string BuildMarkdown(Stage7B7BMhpDirectionFixReport r)
        {
            var sb = new StringBuilder(4096);
            sb.AppendLine("# Stage7B-7B Move / Harvest / Produce Mismatch Audit");
            sb.AppendLine();
            ...
            sb.AppendLine("- cardinal_direction_mapping_mode: " + r.cardinal_direction_mapping_mode);
            sb.AppendLine("- demo_recording_ready_after_7b: " + r.demo_recording_ready_after_7b.ToString().ToLowerInvariant());
            sb.AppendLine();
            sb.AppendLine("## Before / After");
            sb.AppendLine();
            sb.AppendLine("| Metric | Before 7A | After 7B |");
            sb.AppendLine("|---|---:|---:|");
            sb.AppendLine("| overall candidate_match_rate | " + ValueOrNull(r.candidate_match_rate_before_7a) + " | " + ValueOrNull(r.candidate_match_rate_after_7b) + " |");
            sb.AppendLine("| candidate_match_count | " + r.candidate_match_count_before_7a + " | " + r.candidate_match_count_after_7b + " |");
            sb.AppendLine("| Move match_rate | " + ValueOrNull(r.move_match_rate_before_7a) + " | " + ValueOrNull(r.move_match_rate_after_7b) + " |");
            sb.AppendLine("| Move direction_mismatch_count | " + r.move_direction_mismatch_count_before_7a + " | " + r.move_direction_mismatch_count_after_7b + " |");
            sb.AppendLine("| Harvest match_rate | " + ValueOrNull(r.harvest_match_rate_before_7a) + " | " + ValueOrNull(r.harvest_match_rate_after_7b) + " |");
            sb.AppendLine("| Harvest direction_mismatch_count | " + r.harvest_direction_mismatch_count_before_7a + " | " + r.harvest_direction_mismatch_count_after_7b + " |");
            sb.AppendLine("| Produce match_rate | " + ValueOrNull(r.produce_match_rate_before_7a) + " | " + ValueOrNull(r.produce_match_rate_after_7b) + " |");
            sb.AppendLine("| Produce direction_mismatch_count | " + r.produce_direction_mismatch_count_before_7a + " | " + r.produce_direction_mismatch_count_after_7b + " |");
            sb.AppendLine("| Return match_rate | n/a | " + ValueOrNull(r.return_match_rate_after_7b) + " |");
            sb.AppendLine("| Return direction_mismatch_count | n/a | " + r.return_direction_mismatch_count_after_7b + " |");
            sb.AppendLine("| runtime_apply_accept_rate | " + ValueOrNull(r.runtime_apply_accept_rate_before_7a) + " | " + ValueOrNull(r.runtime_apply_accept_rate_after_7b) + " |");
            sb.AppendLine();
            sb.AppendLine("## Mapping Diagnostics");
            sb.AppendLine();
            sb.AppendLine("- mapping_applied_action_types: " + FormatHistogram(r.mapping_applied_action_types));
            sb.AppendLine("- move_mapping_applied_count: " + r.move_mapping_applied_count);
            sb.AppendLine("- harvest_mapping_applied_count: " + r.harvest_mapping_applied_count);
            sb.AppendLine("- return_mapping_applied_count: " + r.return_mapping_applied_count);
            sb.AppendLine("- produce_mapping_applied_count: " + r.produce_mapping_applied_count);
            sb.AppendLine();
            ...
            sb.AppendLine("- action_type_missing_from_candidates: " + r.action_type_missing_from_candidates_count);
            sb.AppendLine();
            sb.AppendLine("## Return");
            sb.AppendLine();
            sb.AppendLine("- return_commands_total: " + r.return_commands_total);
            sb.AppendLine("- return_commands_matched: " + r.return_commands_matched);
            sb.AppendLine("- return_commands_dropped: " + r.return_commands_dropped);
            sb.AppendLine("- return_match_rate: " + ValueOrNull(r.return_match_rate));
            sb.AppendLine("- return_direction_mismatch_count: " + r.return_direction_mismatch_count);
            sb.AppendLine();
            ...
            sb.AppendLine("- remaining_mismatch_reason_histogram: " + FormatHistogram(r.remaining_mismatch_reason_histogram));
            sb.AppendLine();
            sb.AppendLine("## Notes");
            sb.AppendLine();
        ...
        [Serializable]
        private sealed class Stage7B7BMhpDirectionFixReport
        {
            ...
            public string cardinal_direction_mapping_mode;
            ...
            public int state_sync_failed_count_after_7b;
            ...
            public bool demo_recording_ready_after_7b;

            public int candidate_match_count_before_7a;
            public int candidate_match_count_after_7b;
            public float candidate_match_rate_before_7a;
            public float candidate_match_rate_after_7b;
            public float runtime_apply_accept_rate_before_7a;
            public float runtime_apply_accept_rate_after_7b;
            public int runtime_apply_rejected_count_after_7b;

            public List<MetricEntry> mapping_applied_action_types = new List<MetricEntry>();
            public int move_mapping_applied_count;
            public int harvest_mapping_applied_count;
            public int return_mapping_applied_count;
            public int produce_mapping_applied_count;
            ...
            public float move_match_rate_before_7a;
            public float move_match_rate_after_7b;
            public int move_direction_mismatch_count;
            public int move_direction_mismatch_count_before_7a;
            public int move_direction_mismatch_count_after_7b;
            ...
            public float harvest_match_rate_before_7a;
            public float harvest_match_rate_after_7b;
            public int harvest_direction_mismatch_count;
            public int harvest_direction_mismatch_count_before_7a;
            public int harvest_direction_mismatch_count_after_7b;
            ...
            public float produce_match_rate_before_7a;
            public float produce_match_rate_after_7b;
            public int produce_direction_mismatch_count;
            public int produce_direction_mismatch_count_before_7a;
            public int produce_direction_mismatch_count_after_7b;
            ...
            public int produce_type_mismatch_count_after_7b;
            public int action_type_missing_from_candidates_count;
            public int action_type_missing_from_candidates_after_7b;
            ...
            public int return_commands_total;
            public int return_commands_matched;
            public int return_commands_dropped;
            public float return_match_rate;
            public float return_match_rate_after_7b;
            public int return_direction_mismatch_count;
            public int return_direction_mismatch_count_after_7b;
            public float return_direction_mismatch_rate;
            ...
            public List<MetricEntry> remaining_mismatch_reason_histogram = new List<MetricEntry>();
            public List<string> notes = new List<string>();

            public static Stage7B7BMhpDirectionFixReport CreateDefault()
            {
                return new Stage7B7BMhpDirectionFixReport
                {
                    status = "NO_GO",
                    ...
                    return_match_rate = -1f,
                    return_direction_mismatch_rate = -1f,
        ...
        [Serializable]
        private sealed class Stage7B7BRemainingMismatchEntry
        {
        ...
        [Serializable]
        private sealed class Stage7B7BRuntimeApplyTraceEntry
        {
            public int episode_id;
            public int step_id;
            public int command_index;
            ...
            public int raw_teacher_dir = -1;
            public int mapped_unity_dir = -1;
            public string mapping_mode;
            public bool mapping_applied;
        }

        [Serializable]
        private sealed class Stage7B7BMappingTraceEntry
        {
            public int episode_id;
            public int step_id;
            public int command_index;
            ...
            public int action_type;
            public string action_type_name;
            public int raw_teacher_dir;
            public int mapped_unity_dir;
            public string mapped_unity_dir_name;
            public string mapping_mode;
            public bool mapping_applied;
