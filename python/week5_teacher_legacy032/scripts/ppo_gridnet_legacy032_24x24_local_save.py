# Stage 4 patched 24x24 legacy032 training script. Derived from reference reproduction script, but target map/action contract is configurable. Do not use as proof of direct Unity compatibility.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--local-save-model', type=lambda x: bool(strtobool(x)), default=False, nargs='?', const=True,
                        help='save model artifacts locally without requiring wandb')
    parser.add_argument('--local-save-dir', type=str, default=None,
    ...
    parser.add_argument('--expected-map-size', type=int, default=24,
                        help='expected map side length (H and W) for contract verification')
    parser.add_argument('--verify-contract', type=lambda x: bool(strtobool(x)), default=True, nargs='?', const=True,
                        help='verify map/action contract and fail fast when mismatch is detected')
...
def _default_expected_nvec(expected_map_size):
    mapsize = int(expected_map_size) * int(expected_map_size)
    # In MicroRTSGridModeVecEnv attack target branch is local 7x7 = 49, independent of map size.
    # Do not use mapsize as attack branch in GridMode.
    return [mapsize, 6, 4, 4, 4, 4, 7, 49]


def _gridmode_unity_v2_branch_sizes():
    return [6, 4, 4, 4, 4, 7, 49]


def _global_single_action_reference_nvec(expected_map_size):
    mapsize = int(expected_map_size) * int(expected_map_size)
    return [mapsize, 6, 4, 4, 4, 4, 7, mapsize]


def _write_contract_failure_report(reason, details):
    report = {
    ...
    save_dir = _get_local_save_dir()
    os.makedirs(save_dir, exist_ok=True)
    out_path = os.path.join(save_dir, "stage4_contract_failure_report.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2)
    print(f"[stage4] Contract failure report written: {out_path}")


def _get_local_save_dir():
    if args.local_save_dir:
        return args.local_save_dir
...
def save_local_model(agent, global_step, save_dir, checkpoint_paths):
    ...
    metadata = {
        ...
        "gridmode_expected_nvec": _default_expected_nvec(args.expected_map_size),
        "unity_v2_branch_sizes": _gridmode_unity_v2_branch_sizes(),
        "global_single_action_reference_nvec": _global_single_action_reference_nvec(args.expected_map_size),
        "attack_target_semantics": "local_7x7_49",
        "architecture_name": ARCHITECTURE_NAME,
        "mapsize": int(np.prod(envs.observation_space.shape[:2])),
        "num_envs": int(args.num_envs),
        "mask_source": "envs.vec_client.getMasks(0)",
        "script_name": os.path.basename(__file__),
...
class ResolutionAwareDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if tuple(x.shape[-2:]) != self.target_hw:
            # Resolution-aware resize head guarantees actor logits match env HxW contract.
...
class Agent(nn.Module):
    def __init__(self, mapsize):
        ...
        self.actor = ResolutionAwareDecoder(int(envs.action_space.nvec[1:].sum()), target_hw=(h, w))
...
if args.verify_contract:
    expected_nvec = _default_expected_nvec(args.expected_map_size)
    actual_nvec = [int(x) for x in envs.action_space.nvec.tolist()]
    if int(map_h) != int(args.expected_map_size) or int(map_w) != int(args.expected_map_size):
        reason = "BLOCKED_CONTRACT_MISMATCH"
        details = {
            "expected_observation": [int(args.expected_map_size), int(args.expected_map_size), 27],
            "actual_observation": [int(map_h), int(map_w), int(envs.observation_space.shape[2])],
            "expected_action_nvec": expected_nvec,
            "actual_action_nvec": actual_nvec,
        }
        _write_contract_failure_report(reason, details)
        raise RuntimeError(f"{reason}: map shape mismatch for requested contract")
    if actual_nvec != expected_nvec:
        reason = "BLOCKED_CONTRACT_MISMATCH"
        details = {
            "expected_observation": [int(args.expected_map_size), int(args.expected_map_size), 27],
            "actual_observation": [int(map_h), int(map_w), int(envs.observation_space.shape[2])],
        ...
        _write_contract_failure_report(reason, details)
        raise RuntimeError(f"{reason}: action_space.nvec mismatch for requested contract")

agent = Agent(mapsize=mapsize).to(device)

if args.verify_contract:
    with torch.no_grad():
        sample = torch.zeros((1,) + envs.observation_space.shape, device=device)
        actor_out = agent.actor(agent.forward(sample))
    if int(actor_out.shape[1]) != int(map_h) or int(actor_out.shape[2]) != int(map_w):
        reason = "BLOCKED_POLICY_ARCHITECTURE_SHAPE"
        details = {
            "expected_actor_hw": [int(map_h), int(map_w)],
            "actual_actor_hw": [int(actor_out.shape[1]), int(actor_out.shape[2])],
            "map_path": args.map_path,
            "explanation": "Encoder/decoder stride topology from reference script does not preserve requested map size.",
            "minimal_fix_proposal": "Use adaptive pooling or resolution-aware decoder so actor output matches env HxW before action reshape.",
        }
        _write_contract_failure_report(reason, details)
        raise RuntimeError(f"{reason}: actor output shape {list(actor_out.shape)} incompatible with env map {map_h}x{map_w}")
