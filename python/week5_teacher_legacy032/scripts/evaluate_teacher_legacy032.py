...
def main() -> int:
    ...
    if not run["errors"]:
        try:
            state_dict, ckpt_details = load_legacy032_policy_checkpoint(
                checkpoint_path,
                device,
                strict_load=bool(args.strict_load),
            )
            run["checkpoint_format"] = ckpt_details.get("checkpoint_format")
            run["strict_load_status"] = ckpt_details.get("strict_load_status")
