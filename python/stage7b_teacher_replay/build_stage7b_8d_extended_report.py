...
base_report_path = BASE / "stage7b_8c2_padding_warning_fix_report.json"
collect_src_path = BASE / "stage7b_8c2_actual_collect_observations_trace.jsonl"
lifecycle_src_path = BASE / "stage7b_8b6_lifecycle_trace.jsonl"
action_path = BASE / "stage7b_8d_action_trace.jsonl"
runtime_path = BASE / "stage7b_8d_runtime_apply_trace.jsonl"
agent_inventory_src_path = BASE / "stage7b_8c2_agent_inventory.json"
scripted_bot_path = BASE / "stage7b_week7_scripted_bot_throttle_report.json"

report_json_path = BASE / "stage7b_8d_extended_onnx_inference_report.json"
report_md_path = BASE / "stage7b_8d_extended_onnx_inference_report.md"
lifecycle_out_path = BASE / "stage7b_8d_inference_lifecycle_trace.jsonl"
collect_out_path = BASE / "stage7b_8d_actual_collect_observations_trace.jsonl"
agent_inventory_out_path = BASE / "stage7b_8d_agent_inventory.json"
console_export_path = BASE / "stage7b_8d_unity_console_export.json"
...
collect_rows = read_jsonl(collect_src_path)
...
copy_if_exists(lifecycle_src_path, lifecycle_out_path)
copy_if_exists(collect_src_path, collect_out_path)
copy_if_exists(agent_inventory_src_path, agent_inventory_out_path)
...
defensive_pre_ready_observation_count = sum(1 for r in collect_rows if r.get("defensive_pre_ready_observation") is True)
...
phases = [str(r.get("phase", "")) for r in lifecycle_rows]
reset_count = sum(1 for p in phases if p == "MlAgentsTrainingBootstrap.StartNewEpisode.exit")
unexpected_end_episode_count = sum(1 for p in phases if p == "StudentMlAgent.EndEpisode")
...
report = {
    ...
    "exact_generated_artifacts": [
        "python/stage7b_teacher_replay/stage7b_8d_extended_onnx_inference_report.json",
        "python/stage7b_teacher_replay/stage7b_8d_extended_onnx_inference_report.md",
        "python/stage7b_teacher_replay/stage7b_8d_inference_lifecycle_trace.jsonl",
        "python/stage7b_teacher_replay/stage7b_8d_actual_collect_observations_trace.jsonl",
        "python/stage7b_teacher_replay/stage7b_8d_action_trace.jsonl",
        "python/stage7b_teacher_replay/stage7b_8d_runtime_apply_trace.jsonl",
        "python/stage7b_teacher_replay/stage7b_8d_agent_inventory.json",
        "python/stage7b_teacher_replay/stage7b_8d_unity_console_export.json",
    ],
    ...
    "observation_values_written_summary": {
        ...
        "defensive_pre_ready_observation_count": defensive_pre_ready_observation_count,
    },
    ...
    "minimal_next_fix": (
        "Investigate inference decision starvation after first OnActionReceived; ensure DecisionRequester continues scheduling decisions in InferenceOnly mode."
        if blockers
        else "none"
    ),
}
...
md_lines = [
    "# Stage7B-8D Extended ONNX Inference Report",
    "",
    ...
    "",
    "## Core Metrics",
    f"- onnx_asset_path: {report['onnx_asset_path']}",
    ...
    f"- reject_reasons_histogram: {json.dumps(reject_hist_serializable, ensure_ascii=True)}",
    "",
    "## Scripted Bot",
    f"- present: {str(scripted_bot_present).lower()}",
    f"- enabled: {str(scripted_bot_enabled).lower()}",
    f"- decisions_actions_count: {scripted_bot_decisions_actions_count}",
    f"- runtime_apply_attempted: {scripted_bot_runtime_apply_attempted}",
    f"- runtime_apply_accepted: {scripted_bot_runtime_apply_accepted}",
    f"- bot_stopped_unexpectedly: {str(bot_stopped_unexpectedly).lower()}",
    "",
    "## Lifecycle",
    f"- terminal_reached: {str(terminal_reached).lower()}",
    f"- terminal_reason: {terminal_reason}",
    f"- reset_count: {reset_count}",
    f"- unity_console_errors: {unity_console_errors}",
    f"- unity_console_warnings: {unity_console_warnings}",
    "",
    "## Blockers",
]
...
print("Stage7B-8D report generated")
