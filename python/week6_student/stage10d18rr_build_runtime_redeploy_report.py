...
def main() -> int:
    ...
    tmp_dir = root / "python/week6_student/tmp/stage10d18_runtime_redeploy_rerun"
    out_dir = root / "python/week6_student/reports"
    out_dir.mkdir(parents=True, exist_ok=True)

    manifest_path = tmp_dir / "stage10d18_run_manifest.json"
    if not manifest_path.exists():
        raise RuntimeError(f"Missing Stage10D18RR run manifest: {manifest_path}")
    ...
    if not steps:
        raise RuntimeError("No aligned snapshot/cell-table pairs found for Stage10D18RR")
    ...
    adapter_invoked = bool(snapshot_step1.get("adapter_invoked"))
    parsed_logits_available = bool(snapshot_step1.get("parsed_logits_available"))
    python_request_status = str(snapshot_step1.get("python_request_status") or "")
    ...
    binding_labels.append("STAGE10D18RR_CHECKPOINT_BINDING_CONFIRMED" if checkpoint_path_ok else "STAGE10D18RR_CHECKPOINT_BINDING_FAILED")
    binding_labels.append("STAGE10D18RR_REAL_MODEL_LOGITS_CONFIRMED" if (model_loaded and predicted_source == "model_logits" and not fallback_used and not fake_logits_used and not heuristic_policy_path_used) else "STAGE10D18RR_FALLBACK_USED")
    binding_labels.append("STAGE10D18RR_LOGITS_SHAPES_VALID" if (logits_shapes_valid and observation_shape_valid) else "STAGE10D18RR_LOGITS_SHAPES_INVALID")
    ...
    binding_payload = {
        ...
        "checkpoint_binding_status": "STAGE10D18RR_CHECKPOINT_BINDING_CONFIRMED" if binding_ok else "STAGE10D18RR_CHECKPOINT_BINDING_FAILED",
    ...
    binding_path = out_dir / "stage10d18rr_checkpoint_binding_verification.json"
    ...
    if not binding_ok:
        minimal_gate = "GO_FOR_STAGE10D18R_BINDING_FIX_REOPEN"
        trace_path = out_dir / "stage10d18rr_runtime_redeploy_trace.jsonl"
        lifecycle_path = out_dir / "stage10d18rr_produced_unit_lifecycle.json"
        move_path = out_dir / "stage10d18rr_movement_command_path_audit.json"
        action_dist_path = out_dir / "stage10d18rr_action_distribution_over_time.json"
        off_actor_path = out_dir / "stage10d18rr_off_actor_safety_audit.json"
        summary_path = out_dir / "stage10d18rr_visual_behavior_summary.json"
        report_path = out_dir / "STAGE10D18RR_RUNTIME_BC_REDEPLOY_EVAL_RERUN_REPORT.md"
            ...
            json.dumps(
                {
                    ...
                    "off_actor_status": "STAGE10D18RR_OFF_ACTOR_MISLOCALIZATION_DETECTED",
                },
                ensure_ascii=True,
                indent=2,
            ),
            ...
            json.dumps(
                {
                    ...
                    "initial_command_acceptance_confirmed": False,
                    ...
                    "classification_labels": [
                        "STAGE10D18RR_CHECKPOINT_BINDING_FAILED",
                        "STAGE10D18RR_FALLBACK_USED",
                        "STAGE10D18RR_LOGITS_SHAPES_INVALID",
                        "STAGE10D18RR_RUNTIME_REDEPLOY_RERUN_FAIL",
                    ],
                ...
                move_path_rows.append(
                    {
                        ...
                        "p_move": probs["move"],
                        "p_noop": probs["noop"],
                        "p_harvest": probs["harvest"],
                        "p_produce": probs["produce"],
                        "p_attack": probs["attack"],
                        "move_dir": md,
    ...
    off_actor_status = "STAGE10D18RR_OFF_ACTOR_SAFE"
    if off_actor_built_any or off_actor_submitted_any:
        off_actor_status = "STAGE10D18RR_OFF_ACTOR_COMMAND_BUILD_RISK"
    ...
    off_actor_payload = {
        ...
        "classification_labels": [off_actor_status],
    }

    # Primary gate policy from Stage10D.18 rules.
    if not binding_ok:
        primary_next_gate = "GO_FOR_STAGE10D18R_BINDING_FIX_REOPEN"
        ...
        primary_next_gate = "GO_FOR_STAGE10D18_RUNTIME_REDEPLOY_RERUN_INSTRUMENTATION_FIX"

    labels: list[str] = []
    labels.extend(binding_labels)

    if b2_harvest_preserved:
        labels.append("STAGE10D18RR_INITIAL_B2_HARVEST_PRESERVED")
    if c3_produce_preserved:
        labels.append("STAGE10D18RR_INITIAL_C3_PRODUCE_PRESERVED")
    if initial_acceptance:
        labels.append("STAGE10D18RR_INITIAL_COMMAND_ACCEPTANCE_CONFIRMED")
    if not (b2_harvest_preserved and c3_produce_preserved):
        labels.append("STAGE10D18RR_INITIAL_BEHAVIOR_REGRESSED")

    if len(produced_ids) > 0:
        labels.append("STAGE10D18RR_UNITS_PRODUCED_CONFIRMED")
    if produced_units_visible == len(produced_ids) and len(produced_ids) > 0:
        labels.append("STAGE10D18RR_PRODUCED_UNITS_VISIBLE_IN_OBSERVATION")
    if produced_owner_valid == len(produced_ids) and len(produced_ids) > 0:
        labels.append("STAGE10D18RR_PRODUCED_UNITS_OWNER_UNIT_ENCODING_VALID")

    labels.append("STAGE10D18RR_MOVE_PREDICTIONS_PRESENT" if move_predictions_present else "STAGE10D18RR_MOVE_PREDICTIONS_ABSENT")
    labels.append("STAGE10D18RR_MOVE_COMMANDS_BUILT" if move_commands_built else "STAGE10D18RR_MOVE_COMMANDS_NOT_BUILT")
    if move_commands_accepted:
        labels.append("STAGE10D18RR_MOVE_COMMANDS_ACCEPTED")
    elif move_predictions_present:
        labels.append("STAGE10D18RR_MOVE_COMMANDS_REJECTED")
    if move_driven_position_change:
        labels.append("STAGE10D18RR_UNITS_CHANGED_POSITION")
        labels.append("STAGE10D18RR_MOVE_DRIVEN_POSITION_CHANGE_CONFIRMED")
    else:
        labels.append("STAGE10D18RR_NO_MOVE_DRIVEN_POSITION_CHANGE")

    if behavior_progress_beyond_production:
        labels.append("STAGE10D18RR_BEHAVIOR_PROGRESS_BEYOND_PRODUCTION")
    if economy_only_behavior:
        labels.append("STAGE10D18RR_ECONOMY_ONLY_BEHAVIOR_PERSISTED")
    if produce_loop_no_movement:
        labels.append("STAGE10D18RR_PRODUCE_LOOP_NO_MOVEMENT_PERSISTED")
    labels.append("STAGE10D18RR_ATTACK_BEHAVIOR_PRESENT" if attack_behavior_present else "STAGE10D18RR_ATTACK_BEHAVIOR_ABSENT")

    labels.append(off_actor_status)

    units_changed_any_count = 0
    for hist in unit_hist.values():
        if not hist:
            continue
        start_xy = (hist[0]["x"], hist[0]["y"])
        if any((h["x"], h["y"]) != start_xy for h in hist[1:]):
            units_changed_any_count += 1

    if primary_next_gate in ("GO_FOR_STAGE10D19_EXTENDED_TACTICAL_EVALUATION", "GO_FOR_STAGE10D19_ATTACK_BEHAVIOR_AUGMENTATION") and off_actor_safe:
        labels.append("STAGE10D18RR_RUNTIME_REDEPLOY_RERUN_SUCCESS")
        runtime_mode = "success"
    elif primary_next_gate in ("GO_FOR_STAGE10D19_EXTENDED_TACTICAL_EVALUATION", "GO_FOR_STAGE10D19_ATTACK_BEHAVIOR_AUGMENTATION"):
        labels.append("STAGE10D18RR_RUNTIME_REDEPLOY_RERUN_PARTIAL_SUCCESS")
        runtime_mode = "partial_runtime_progress_with_off_actor_risk"
    elif primary_next_gate == "GO_FOR_STAGE10D18R_BINDING_FIX_REOPEN":
        labels.append("STAGE10D18RR_RUNTIME_REDEPLOY_RERUN_FAIL")
    ...
    visual_summary = {
        ...
        "initial_command_acceptance_confirmed": initial_acceptance,
        ...
        "total_move_commands_built": move_commands_built_total,
        "total_move_commands_accepted": move_commands_accepted_total,
        "units_that_changed_position_count": units_changed_any_count,
    ...
    report_lines.append("# STAGE10D18RR_RUNTIME_BC_REDEPLOY_EVAL_RERUN_REPORT")
    report_lines.append("")
    report_lines.append("## 1. Purpose and constraints")
    report_lines.append("- Runtime redeploy evaluation only after Stage10D.18R binding fix.")
    report_lines.append("- No PPO, no teacher/student training, no checkpoint mutation, no runtime semantic changes.")
    report_lines.append("")
    report_lines.append("## 2. Stage10D.18 failure recap")
    report_lines.append("- Original Stage10D.18 failed due to binding/bridge startup path; runtime behavior evaluation was blocked.")
    report_lines.append("")
    report_lines.append("## 3. Stage10D.18R binding fix recap")
    report_lines.append("- Filename gate fix enabled Stage10D.17 checkpoint basename acceptance in Unity adapter.")
    report_lines.append("- Stage10D.18R confirmed real model logits path and no fallback.")
    report_lines.append("")
    report_lines.append("## 4. Checkpoint/inference binding verification")
    ...
    report_lines.append(f"- adapter_invoked: {adapter_invoked}")
    report_lines.append(f"- parsed_logits_available: {parsed_logits_available}")
    report_lines.append(f"- model_loaded: {model_loaded}")
    report_lines.append(f"- python_request_status: {python_request_status}")
    ...
    report_lines.append("")
    report_lines.append("## 5. Run configuration")
    ...
    report_lines.append("")
    report_lines.append("## 6. Initial Harvest/Produce regression check")
    ...
    report_lines.append("")
    report_lines.append("## 7. Produced unit lifecycle")
    ...
    report_lines.append("")
    report_lines.append("## 8. Movement command path audit")
    ...
    report_lines.append("")
    report_lines.append("## 9. Action distribution over time")
    report_lines.append(f"- temporal_pattern_labels: {temporal_labels}")
    report_lines.append("")
    report_lines.append("## 10. Off-actor safety audit")
    ...
    report_lines.append("")
    report_lines.append("## 11. Visual behavior summary")
    ...
    report_lines.append("")
    report_lines.append("## 12. Classification labels")
    for label in labels:
        report_lines.append(f"- {label}")
    report_lines.append("")
    report_lines.append("## 13. Primary next gate")
    report_lines.append(f"- {primary_next_gate}")
    report_lines.append("")
    report_lines.append("## 14. What not to do next")
    ...
    report_lines.append(f"- Was fallback avoided? {not fallback_used and not fake_logits_used and not heuristic_policy_path_used}")
    report_lines.append(f"- Were logits shapes valid? {logits_shapes_valid and observation_shape_valid}")
